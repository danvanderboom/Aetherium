...
## Purpose
Defines agent performance telemetry: per-step performance snapshots, replay storage for failed runs, and performance analysis that identifies weaknesses and recommendations — stored in an Orleans telemetry grain and collected automatically during agent execution.
