...
- [ ] 1.1 `Aetherium.Model/Eca/EcaVocabularyTypes.cs`: `EcaTileRole`, `EcaValueType`, `EcaParameter`,
      `EcaTileDefinition` — pure metadata types the registry, validator, and doc-gen share.
- [ ] 1.2 `Aetherium.Model/Eca/EcaConfig.cs`: `EcaConfig`, `EcaRule` (string `When`),
...
- [ ] 1.3 Add `Rules` to `GameDefinition`; `EcaConfig` to `WorldTemplate`, `WorldConfig`,
      `CreateWorldRequest` (nullable, additive serializer ids).

## 2. Vocabulary registry

- [ ] 2.1 `Aetherium.Server/Eca/Vocabulary/`: one tile type per trigger/condition/action
...
- [ ] 2.2 `EcaVocabulary`: `Id → EcaTileDefinition` registry (`All`, `TryGet`, by-role views);
      the single source of truth for validation, docs, and the runtime switch.

## 3. Loader / validator / mapper

- [ ] 3.1 `GameDefinitionLoader`: `rules.yaml` conventional sibling (+ inline `rules:`),
      duplicate-declaration rejection, strict parsing.
- [ ] 3.2 `GameDefinitionValidator`: registry-driven — unique rule ids; `Kind` ∈ vocabulary; required
...
- [ ] 3.3 `GameDefinitionMapper`: carry `Rules` onto `CreateWorldRequest`.

## 4. Runtime evaluator + grain wiring

- [ ] 4.1 `Aetherium.Server/Eca/EcaRuntime.cs`: pure `Evaluate(EcaEventContext) →
...
- [ ] 4.2 `GameMapGrain`: thread `ecaConfig` through `InitializeAsync` → `MapState`; `ApplyEcaConfig`
      on init + reactivation; seed the runtime RNG from the world seed.
- [ ] 4.3 Raise `creature_died` at the shared monster-defeat branch (both `AttackAsync` and
...
- [ ] 4.4 Thread through `WorldGrain`, `GameManagementGrain` (both paths), `OrleansWorldHost`.

## 5. Docs

- [ ] 5.1 `Aetherium.Server/Eca/EcaVocabularyDoc.cs`: `GenerateMarkdown()` renders `EcaVocabulary` as a
      reference table (tile, role, description, parameter schema).
- [ ] 5.2 `docs/eca-scripting.md`: hand-written language guide (fit vs the T5 vision, rule anatomy,
...
- [ ] 6.1 Emberfall `rules.yaml`: "on creature_died, if cult_acolyte + chance 0.5, spawn wolf"
...
- [ ] 7.1 Vocabulary: `EveryTileHasUniqueIdAndParameters`, `RuntimeKinds_MatchVocabularyIds`.
- [ ] 7.2 Loader: `LoadBundle_RulesSection_BindsTriggersConditionsActions`.
- [ ] 7.3 Validator: duplicate ids / unknown spawn creature / unknown status / chance range /
      creature_type_is warning.
- [ ] 7.4 Evaluator (pure): no-conditions-fires, creature_type gating, chance 0/1, target resolution.
- [ ] 7.5 Grain integration: death triggers spawn rule, deal_damage-to-killer applies, no-config
      kill path unchanged, rules recompile on reactivation.
- [ ] 7.6 Docs: `CommittedDoc_MatchesGeneratedVocabularyReference`.
- [ ] 7.7 Full suite green (Aetherium.Test + Aetherctl.Test).
