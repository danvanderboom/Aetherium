...
Aetherium uses spec-driven development. `openspec/specs/<capability>/spec.md` is current truth (~38 capabilities: engine-core, perception, perception-vision, client-server-communication, console-view, world-building, world-entities, pcg-* (7), narrative, interaction, audio, geometry-maze, game-management-grain, demo-game, multiworld, meta-progression, agent-telemetry, training-* (3), plus **flight, combat, character-memory, identity-recognition, eca-scripting, content-definitions, instances, simulation-runtime, world-persistence, aetherctl, client** — added when the completed-change backlog was archived on 2026-07-19); `openspec/changes/<id>/` holds proposals — now genuine **forward proposals** (abilities, factions, NPC behavior trees, continuous action pipeline, boardable vehicles, arcade-client, …) plus a few in-progress `wire-*` changes. Note `add-arcade-client` is a design proposal (1/29 tasks), not yet built, and `add-xbox-controller-unity` (complete) stays active until `add-unity-2d-client` is archived first. Conventions and the full workflow: [openspec/AGENTS.md](../../openspec/AGENTS.md) and [openspec/project.md](../../openspec/project.md). Note: the `openspec` CLI is not required to read/edit specs — it validates and archives changes when available.
