...
> **Reconciliation — `develop` @ 2026-07-03, updated after Phase 1 (commit `cd6cf67`).** This audit was written at baseline `5b7e267`; a durable-persistence workstream (snapshot + delta-log + SQLite + compaction) and a driven world-tick service landed afterward (`d01fb94`), followed by a Phase-1 foundation pass. **BOTH CRITICAL FINDINGS NOW FIXED:** the boot hang — the whole self-referential DI bridge (`Program.cs:276-348`, not just the `IGrainFactory` piece) was deleted, and a boot test confirms the server now boots with Orleans enabled (startup banner + `Started silo`, 0 exceptions). This surfaced a *second*, previously-masked defect: `AddMemoryStreams("Default")` requires a `PubSubStore` grain storage that was never registered — fixed in the same change. `metaStore` is now registered alongside it. **Also fixed pre-Phase-1:** GameMapGrain world persistence (now snapshots the World and restores it via `SnapshotWorldBuilder` + `MapDeltaReplayer` on reactivation), the undriven tick pipeline (`WorldTickService` is now a real hosted service), and the silent `ORLEANS_STORAGE≠memory` no-storage case. **FIXED IN PHASE 5:** the tick pipeline now *drives NPC behavior* — `GameMapGrain.TickAsync` steps every monster one validated cardinal cell and fans out the move, so the substantial-but-previously-dead tick machinery finally produces visible gameplay (monsters wander and are perceived by co-located players). **FIXED IN PHASE 5 (P3-5):** the AgentRunnerGrain off-scheduler loop — `RunAsync` now uses an Orleans grain timer (`RegisterGrainTimer`) so steps run on the activation turn, not a thread-pool `Task.Run`. **STILL STANDS (Medium/High):** WorldGrain reentrant RMW races, `WorldGrainState`/`ClusterState` missing `[GenerateSerializer]` (now higher-risk under SQLite serialization), unbounded telemetry, EventScheduler duplication. **New persistence findings:** heat state is not persisted (cold-start loses accumulated heat trails); `PersistDeltaAsync` swallows persistence failures silently (a player can see non-durable state); SQLite writer-writer concurrency and delta-log GC are weakly tested; `WorldSnapshot` carries no embedded schema-version field (only deltas are version-guarded). Net: the server is now reachable with Orleans on, which makes all of the above — previously moot — live production risk. Per-finding detail is in the Reconciliation section at the end.
