...
> **Reconciliation note (`develop` @ 2026-07-03, updated after Phase 2, commits `889878f`+`253078a`).** This analysis was written at baseline `5b7e267`. The 23 commits that landed afterward (reconciled in `d01fb94`) moved the themes below in an encouraging direction; a Phase-1 pass (`cd6cf67`) closed the foundation blockers; and a Phase-2 pass (`889878f` + hardening `253078a`) closed the live-path correctness theme. Theme 1 (breadth over depth) is being actively reversed: the multiplayer/persistence/travel work is exactly the "start building the bridges" the strategic recommendation called for, shipped with real integration tests. Theme 4 (aspirational persistence) is largely resolved. **Theme 3 (authoritative server that doesn't validate) is now RESOLVED** — the eye-opener was that `ObstructsMovement` was written by walls/doors but never read, so *nothing blocked movement on any path*; movement is now validated on all three paths, interactions are reach-checked, and the mutating REST surface requires auth. Theme 5 (no safety net) is **half-resolved**: the defects CI would have caught are fixed by hand, but CI itself is still absent (deliberately deferred, a cost decision) — so nothing prevents a silent regression. **Theme 2 (parallel implementations) is now the top open design issue**: there are still **three** action paths (obsolete hub methods, `LocalMutationGateway`, `GrainMutationGateway`); Phase 2 made all three *correct*, but they should be *converged* onto one owner (P1-4) so the correctness can't drift apart again. Net read: the foundation and live-path gates this document prioritized are open; the next strategic move is convergence + CI ([IMPROVEMENT_PLAN.md](IMPROVEMENT_PLAN.md) Phase 3), not more feature breadth.
