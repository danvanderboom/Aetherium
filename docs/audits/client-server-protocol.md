...
> **Reconciliation — `develop` @ 2026-07-03.** The grain-authoritative multiplayer work landed a delta protocol and grain-hosted worlds. **FIXED:** reconnect/resume now works (the `GameClient` reconnect state machine re-raises `Connected` via the `Reconnected` path, and grain-bound sessions survive because the grain holds canonical state); `JoinWorld` now binds a session to a grain world (resolves grains, `JoinPlayerAsync`, snapshot hydration, gateway swap) instead of erroring and zombie-ing the client; and the `GameHub.ExecuteTool` authorization bypass is closed (the hub now checks `AgentToolProfile.Player.IsToolAllowed`). **FIXED IN PHASE 2 (`889878f` + hardening `253078a`):** the Critical no-server-side-movement-validation finding is closed on **all three** paths — a shared engine rule (`World.MovementBlocker`/`TryMoveSteps`/`TryChangeLevel`) validates passability, occupancy, map bounds, and stair placement, and `GameMapGrain.MoveAsync` now also enforces `MoveTool`'s 1–100 clamp (the root cause was that `ObstructsMovement` was written but never read). Map-wide door/use is closed by an interaction reach rule (incl. Z). The **anonymous REST control-plane** is fixed — `ApiKeyMiddleware` now gates mutating requests to the six previously-open controllers. **Cross-path races** for legacy sessions are serialized via `GameSession.WithStateLock` (grain-bound sessions were already Orleans-serialized). **STILL STANDS:** ManagementHub-without-B2C 500s, absent wire versioning, the `AmbientTint` tuple loss, and — the top remaining design item — the **three coexisting action paths** (obsolete hub methods + `LocalMutationGateway` + `GrainMutationGateway`): Phase 2 made all three *correct*, but converging them onto one owner (P1-4) is still open so the correctness can't drift apart again. The delta protocol stays cleanly perception-pure (filtered `PerceptionDto`s ordered by a monotonic per-map `Sequence`). Detail in the Reconciliation section at the end.
