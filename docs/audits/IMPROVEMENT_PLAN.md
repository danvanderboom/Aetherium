...
> **Reconciliation note (`develop` @ 2026-07-03, updated after Phase 2, commits `889878f`+`253078a`).** Work on `develop` had advanced parts of this plan out of order (Phase 5 items landed before the foundation), then two sequenced passes closed the foundation and the live-path layer. **Phase 1 done, except CI** (`cd6cf67`): server boots with Orleans, full-solution build green, runtime mismatch moot (net10.0), quick-win correctness bugs fixed. **Phase 2 done** (`889878f` + hardening `253078a`): movement is validated on all three paths (`World.MovementBlocker`/`TryMoveSteps`/`TryChangeLevel` — the root cause was that `ObstructsMovement` was written but never read); interactions are reach-checked; the mutating REST control-plane requires the API key; infrared renders (plus a latent crash fixed); and the inventory-capacity exploit, `MoveEntity` index-drop, and legacy-session cross-path races are closed. Suite at **910+31 / 0 skipped**. **Pre-existing (out-of-order) progress:** durable persistence, multi-world travel, observer perception (Phase 5 items 1/3/6), driven tick pipeline (Phase 3), and +130 integration/determinism tests (Phase 4). **Still open:** **CI (P2-1)** remains deliberately deferred (project cost decision) — the one thing that would stop a silent regression of all the above; treat `dotnet build && dotnet test` as a manual pre-merge habit. **Recommendation: Phase 3 next** — converge the now-three action paths onto one owner (P1-4), delete the ~6k lines of dead legacy client code (P1-8), and reconcile the remaining duplicates — before re-opening Phase-5 feature deepening.
