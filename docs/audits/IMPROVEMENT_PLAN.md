...
> **Reconciliation note (`develop` @ 2026-07-03, updated after Phase 4; see also the Phase-4 status line in its section below).** Work on `develop` had advanced parts of this plan out of order (Phase 5 items landed before the foundation), then three sequenced passes closed the foundation, the live-path layer, and the debt layer. **Phase 1 done, except CI** (`cd6cf67`): server boots with Orleans, full-solution build green, runtime mismatch moot (net10.0), quick-win correctness bugs fixed. **Phase 2 done** (`889878f`+`253078a`): movement validated on all three paths, interactions reach-checked, REST control-plane authed, infrared renders, capacity/index/race hardening. **Phase 3 done** (`1c32327`): the dead legacy console engine is deleted (~6.3k lines), one shared worldgen pass list feeds the grain/snapshot/CLI paths (server worlds are now themed and populated), the action-path convergence turned out to be already landed (`9d2e150` had removed the obsolete hub methods; the item-narrative-event parity gap is now closed), `WorldTickService`'s fate is decided (it stays; NPC ticking is Phase-5 feature work), stub tools decline honestly, unbounded collections are capped, `MapValidator` runs on live worlds, and all active OpenSpec `tasks.md` files reflect verified reality. Suite now at **928+38 / 0 skipped** after Phase 4 (test depth: FOV-rotation regression, real CLI tests, client-side tests, travel integration; plus the P1-10/P1-11 console-rendering fixes). **Still open:** **CI (P2-1)** remains deliberately deferred (project cost decision); P2-7 (TestCluster sharing, deliberately skipped); P1-13 (simulation thread-safety), P1-21 (script bugs). **Phase 5 first slice done** — NPCs are now visible and ticking (the "make the game a game" milestone): monsters wander on the world tick (`GameMapGrain.TickAsync`) and render on the client (`PerceptionDto.VisibleCharacters`), covered by +9 tests (suite 937+38 / 0 skipped), boot-verified. **Phase 5 slices 1–2 done** — NPCs tick and render (`add-npc-behavior`), and an agent plays the live shared map on a grain timer (`add-agent-live-map-play`). **Recommendation: continue Phase 5** — durable persistence is already largely 🟢 (P3-8); next high-value items are PCG placement & difficulty (P3-6), narrative/quest activation (P3-2), then deeper NPC AI/combat (P3-7). LLM decision-quality + prompt-template feeding is a follow-up on the agent slice.
...
**Status (2026-07-03, commit `cd6cf67`): done except CI.** Every item below is fixed and verified (build 0 errors, boot succeeds with Orleans on, suite 865+31 passing / 0 skipped) except **P2-1 (CI)**, which is deliberately deferred per a project cost decision rather than pending work. P1-21 (script bugs) was not addressed in this pass. See the reconciliation note above and [RECOMMENDATIONS.md](RECOMMENDATIONS.md) for verification detail on each item.
...
- **Restore the toolchain:** P1-2 (`global.json`/`RollForward` for the runtime mismatch — turned out moot; `develop` migrated to net10.0 instead), P1-19 (drop the stale Orleans pin), P1-21 (script bugs — not addressed).
- **Stand up CI:** P2-1 (`dotnet build && dotnet test` on `main`) — **deferred**, a project cost decision.
...
**Status (2026-07-03, commits `889878f` + `253078a`): done.** The exit criteria are met — a player can no longer walk through walls, act at a distance, dupe items via re-equip, or hit the mutating REST surface anonymously; infrared renders; the legacy-session races are serialized; and each fix has targeted tests (suite 910+31 / 0 skipped). Two items in the list below were *already* done before this pass (P0-6 hub auth, P0-11 reconnect); the console-rendering polish (P1-10/P1-11) and the broader thread-safety of the simulation singletons (P1-13) were **not** in scope here and remain open as debt items. See the reconciliation note above and [RECOMMENDATIONS.md](RECOMMENDATIONS.md) for per-item detail.
...
**Status (2026-07-03, commit `1c32327`): done.** P1-4 was found largely pre-landed (`9d2e150` removed the obsolete hub methods; the item narrative-event parity gap is now closed); P1-6, P1-8, P1-12 (decision), P1-14, P1-15, P1-20, and P1-22 all landed and are verified (build 0 errors, 914+31 tests / 0 skipped, boot-tested). The "consider extracting a shared engine library" item is moot — the duplicated console engine was dead code and was deleted rather than shared. See [RECOMMENDATIONS.md](RECOMMENDATIONS.md) for per-item detail.
...
**Status (2026-07-03): done except P2-1 (CI, deliberately deferred) and P2-7 (TestCluster sharing, deliberately skipped — Low severity, touches 16 fixtures, stability risk for a speed win).** P2-4 (FOV rotation regression), P2-5 (real CLI invocation tests), P2-6 (client-side tests via extern alias), and P2-2's completable extent (connect→act→observe→travel with perception isolation; quest/instance legs blocked by P3-2/P3-4) all landed, plus the P1-10/P1-11 console-rendering fixes the client tests interacted with, and a 2s→15s `PhaseTimeout` fix that was intermittently failing real generations under load. P2-9/P2-10 were already done. Suite at **928+38 / 0 skipped**.
...
1. **Single-world exploration slice** — P3-1 (perception to observers) + the Phase 2 movement/interaction work makes co-op exploration real; make NPCs visible and ticking (depends on P1-12). This is the "make the game a game" milestone. **✅ Done (2026-07-03):** monsters wander on the world tick and render on the client (`add-npc-behavior`); +9 tests, boot-verified. Deeper NPC AI (aggro/pursuit/combat) is folded into P3-7.
2. **Agent play** — P3-5 (agent↔game integration on the runner, prompt-driven decisions) so an LLM agent can play the slice; leverages the strong existing tool system. **✅ Done (2026-07-03):** an agent joins a live map as a Character and plays the shared world visibly to players (`AttachToWorldAsync` + `GrainMutationGateway`), the run loop moved onto a grain timer, reachable via `aetherctl agent attach-world`, integration-tested with the heuristic policy (no LLM needed) — `add-agent-live-map-play`. Follow-up: feed prompt templates into LLM decisions and improve LLM decision quality.
