...
3. **Fixed (Phase 5)** — the historical `Monster.GetValidDirections` enumeration-mutation crash was already fixed; the two remaining hazards are now closed too: the empty-list `rand.Next` throw when a monster is boxed in is gone (`NextWanderDirection` returns `null` instead), and `Monster.Heartbeat` is no longer dead — it delegates to `NextWanderDirection`, and the tick pipeline drives monster movement through the grain (which owns broadcasting) rather than `Heartbeat` directly.
