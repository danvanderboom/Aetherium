...
> **Reconciliation — `develop` @ 2026-07-03.** A large remediation (commit `af66c28`, "comprehensive procedural generation remediation Phases 1-5", ~60 fixes + 69 new tests) landed after the baseline. It substantially improved *algorithm correctness, determinism, and testing*: **FIXED** — the determinism gap now has a real byte-for-byte world-hash test (`Aetherium.Test/WorldGen/DeterminismTests.cs`: same seed → same hash, plus `EffectiveSeed` replay), and mid-pipeline error handling now cleanly separates pass-abort from validation outcome (`WorldGenerationOrchestrator` exposes `AbortedByPass`, tested). MST/Prim's, secret-room retry, river-carver whitelist, portal-network typing/reachability, and generator-registry reflection hardening all improved. **Of the three structural HIGH findings, one is now FIXED (Phase 1, commit `cd6cf67`):** the server now calls `MapGeneratorRegistry.DiscoverTypes` at construction, so it honors the requested generator type instead of always falling back to `AdvancedDungeonGenerator`. **The other two STILL STAND:** placement/population features (`SpawnNPCsFeature`, `ItemDistributionFeature`, `PrefabStamper`, `AdaptationPass`) are still console-log stubs; generator parameters still have no effect (`AdvancedDungeonGenerator` reads zero `GeneratorParams`; the curriculum/difficulty system remains decorative). **Also fixed in Phase 1:** `PrefabLibrary.LoadFromDirectory` is now wired (`Data/Prefabs/**/*.json` load at startup), and the A/B seed bug (auto-seeded candidates now vary by index instead of all sharing one seed). **Still stands:** the three divergent pass lists (server produces emptier worlds than CLI/dashboard preview), hybrid anchors ignored, narrative tokens are a facade, `MapValidator` still not run on live session worlds. Net: the generation *core* is robust and well-tested, the server now respects the requested generator and loads prefabs, but placement/parameters remaining stubbed still means a **player** sees an unpopulated, parameter-inert world.
