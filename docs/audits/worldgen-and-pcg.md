...
> **Reconciliation — `develop` @ 2026-07-03.** A large remediation (commit `af66c28`, "comprehensive procedural generation remediation Phases 1-5", ~60 fixes + 69 new tests) landed after the baseline. It substantially improved *algorithm correctness, determinism, and testing*: **FIXED** — the determinism gap now has a real byte-for-byte world-hash test (`Aetherium.Test/WorldGen/DeterminismTests.cs`: same seed → same hash, plus `EffectiveSeed` replay), and mid-pipeline error handling now cleanly separates pass-abort from validation outcome (`WorldGenerationOrchestrator` exposes `AbortedByPass`, tested). MST/Prim's, secret-room retry, river-carver whitelist, portal-network typing/reachability, and generator-registry reflection hardening all improved. **BUT the three structural HIGH findings STILL STAND** and keep the *in-game* worlds empty/static: (1) the server still never calls `MapGeneratorRegistry.DiscoverTypes` (`Program.cs`), so the game server ignores the requested generator type and always falls back to `AdvancedDungeonGenerator`; (2) placement/population features (`SpawnNPCsFeature`, `ItemDistributionFeature`, `PrefabStamper`, `AdaptationPass`) are still console-log stubs; (3) generator parameters still have no effect (`AdvancedDungeonGenerator` reads zero `GeneratorParams`; the curriculum/difficulty system remains decorative). Also STANDS: the three divergent pass lists (server produces emptier worlds than CLI/dashboard preview), hybrid anchors ignored, narrative tokens are a facade, `PrefabLibrary.LoadFromDirectory` still unwired, `MapValidator` still not run on live session worlds, A/B seed bug. Net: the generation *core* went from "decorative" to "robust and well-tested," but nothing changed that would make a **player** see a populated, parameter-driven, correctly-typed world.
