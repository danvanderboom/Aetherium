...
> **Reconciliation — `develop` @ 2026-07-03.** The grain-authoritative delta model changed the multiplayer perception story. **PARTIAL FIX (the big one):** "observers never receive perception updates" is now fixed *for grain-hosted maps* — `GameMapGrain.FanOutAsync` broadcasts deltas to the map's SignalR group, `GameSessionManager.NotifyMapMutationAsync` applies each delta to every co-located session's local mirror and recomputes that session's FOV-filtered perception, so co-located players now see each other move. Legacy (non-`JoinWorld`) single-world sessions still get no observer updates. Heat handling also improved: it's now grain-authoritative (`GameMapGrain._heatTracker`, single-threaded by activation) and replayed to sessions via `HeatRecordedDelta`, so the `HeatTrailTracker` thread-safety finding is PARTIAL-fixed for the grain path. **FIXED IN PHASE 2 (`889878f`):** infrared no longer renders black — `ComputeHeatVision` records each heated cell's intensity into the light frame, so `VisualDto.LightLevel` (the heat channel infrared clients color by) carries real heat instead of 0.0. The regression test also exposed and fixed a latent crash: `InfraredVisionSystem` used `Component.Get<T>()`, which *throws* when the component is absent, so infrared perception crashed on any entity without a heat signature — infrared had never actually worked in a real world. **FIXED IN PHASE 4:** the FOV rotation-invariance regression test now exists (`FovRotationInvarianceTests`: omnidirectional visible set + per-key terrain stable across cardinal and non-cardinal headings; directional cone is a strict rotating subset; mode toggle restores the identical set) — the single highest-value gap this audit identified is closed, and the fixed-by-design rotation bug can no longer silently return. **FIXED IN PHASE 5:** NPCs/monsters are now drawn on the client map — perception carries visible characters as structured, renderable data (`PerceptionDto.VisibleCharacters`, each with the entity's tile), and `ClientConsoleMapView` renders characters above items above terrain via a pure, tested `ResolveContentLayer`; a monster no longer shows as the terrain beneath it. The slice also hardened two more latent `Component.Get<Inventory>()` crashes in the perception path (the same `Get<T>`-throws hazard as the infrared bug), which would have crashed inventory/navigation computation for any character at the perceived cell without an inventory — now live, since NPCs are inventory-less Characters. **STILL STANDS:** lighting modes still mutually exclusive, and the post-hoc sunlight no-op. Detail in the Reconciliation section at the end.
...
**[Medium · Verified — FIXED IN PHASE 5] NPCs/monsters are never drawn.** *(Original finding:)* The server encodes character presence only as `ThingsSeen` counts (`VisionSystem.cs:103-116`); `ClientConsoleMapView.DrawContents` renders player, items, and terrain only (`:136-182`) and never reads `ThingsSeen`. A visible monster shows as its underlying terrain tile. *(Fix:)* `PerceptionService` now also emits `PerceptionDto.VisibleCharacters` (parallel to `VisibleItems`), each carrying the character's `TileType`; the client draws characters above items above terrain, excluding the perceiving player. The `ThingsSeen` count remains for AI/analytics but is no longer the only character signal.
