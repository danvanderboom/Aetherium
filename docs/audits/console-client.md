...
> **Reconciliation — `develop` @ 2026-07-03 (updated after Phase 4).** **FIXED (Phase 4):** the three High/Medium runtime bugs are closed — rendering is serialized behind a render lock (torn frames, P1-10); `StatusMessage` finally renders (a STATUS panel — feedback was invisible) and inventory/status text flows through escaped markup builders so `[gold-key]`-style ids can't crash the frame (P1-11); and `GameClient`'s last-resort manual restart now re-raises `Connected` (the residual half of the reconnect soft-lock) with a non-zero retry delay. First real client-side unit tests landed (`ClientWidgetAndMarkupTests`, via the extern alias); the aspirational `CLIENT_TESTS_README.md` is deleted. **FIXED (Phase 3):** the dead legacy engine is deleted (~6.3k lines) — the project is now just the live SignalR client; the legacy `ConsoleMapView` rotation trap and `Monster.cs` `NotImplementedException` findings are moot. **FIXED (earlier):** the auto-reconnect soft-lock, the dead input block. **STANDS:** the audio findings (no shipped assets, reverb stub, dead null-fallback) and monitoring findings (`ConcurrentBag` leak, slow-consumer hazards). **Minor:** during a disconnect the loop sets a `"Reconnecting…"` status but never re-renders until the next perception/command.
