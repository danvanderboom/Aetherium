...
> **Reconciliation — `develop` @ 2026-07-03 (updated after Phase 3, `1c32327`).** **FIXED (Phase 3):** the dead legacy code is **deleted** — `ConsoleDungeonGame`, `ClientConsoleDungeonGame`, and the entire duplicate client-side engine (World/Views/Perception/Lighting/WorldBuilders/Entities/Components/Geometry, ~6.3k lines) are gone; `Enums.cs`/`Extensions.cs` are trimmed to their live members. The project is now just the live SignalR client and its rendering/audio/monitoring stack, and the legacy `ConsoleMapView` rotation trap no longer exists. The `Monster.cs:78 NotImplementedException` finding is moot (file deleted). **FIXED (earlier):** the reconnect soft-lock (`GameClient` re-raises `Connected`), and the dead input block (Phase 1: M / Shift+M work). **STANDS:** unsynchronized torn-frame rendering (P1-10), invisible `StatusMessage` + inventory markup injection (P1-11), and the audio/monitoring findings. **Minor:** during a disconnect the loop sets a `"Reconnecting…"` status but never re-renders, so it stays invisible until the next perception/command.
