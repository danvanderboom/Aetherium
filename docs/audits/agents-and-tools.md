...
- **`AgentRunnerGrain` violates Orleans single-threaded execution — FIXED (Phase 5).** *(Original:)* `RunAsync` spawns `Task.Run` calling `StepAsync()` on the grain instance from a thread-pool thread, mutating grain state outside the scheduler and racing `GetStatusAsync`/`DetachAsync`; the loop keeps running after deactivation. *(Fix:)* `RunAsync` now registers an Orleans grain timer (`RegisterGrainTimer`, `Interleave = false`); each step runs on the grain's activation turn, serialized with all other calls, and the timer self-disposes on the step budget / Stop / Detach.
