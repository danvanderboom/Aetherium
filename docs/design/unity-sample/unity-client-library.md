...
- **`GridMapView` + `EntityViewRegistry`** — subscribe to store events and materialize views: reveal terrain cells as placed prefabs, spawn entity views on `EntityAppeared`, tween on `EntityMoved` (duration scaled to the gap between frames so ≈1 Hz NPC steps read as deliberate motion, not teleports), play a vanish/death dissolve on `EntityVanished`, dim remembered cells. These are *defaults* — a game can replace either wholesale and talk to the store directly; Aphelion uses them as the base and layers its own VFX.
  ...
  **Cell placement is topology-aware** (docs/grid-topologies.md): the server names the world's tiling in `PerceptionDto.Topology` (`square`/`hex`/`tri`, later `h3`), and `GridMapView` converts each relative cell to local space through **`Aetherium.Model.GridCellLayout.CellLayoutPosition`** — the shared, unit-tested layout math (square identity; hex pointy-top axial `x = q + r/2, y = r·√3/2`; triangle half-width columns with the up/down orientation derived from `PerceptionDto.SelfCellParity`). The positions mirror the server topologies' own `CellCenter` embeddings (adjacent centers exactly one unit apart), so client visuals and server geometry (FOV cones, light falloff) always agree about where a cell is; games scale by their own cell size. The console client's `ClientConsoleMapView` drives its character grid from the same helper.
