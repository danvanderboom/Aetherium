...
### P1 — hexagon *(M/L)* — ✅ **topology built; dedicated generator/bundle remain as polish**
...
Because hex reuses `WorldLocation(q,r)` and every generator fills integer coordinates, a hex world is **already creatable through the existing world-creation path with any generator** — the topology reinterprets adjacency; the terrain is valid either way. **Remaining (layout polish, not correctness):** a dedicated `HexCavesGenerator` declaring `SupportedTopologies=["hex"]` for hex-shaped caves, the generator-supports-topology validator check (needs the generator instance the pass pipeline resolves), a `hexhaven` bundle under `Data/Games/`, a full grain-level bundle-boot test, and the client-library `GridMapView` hex layout note. Full hex rationale: [hexagonal-tiles.md](hexagonal-tiles.md).
