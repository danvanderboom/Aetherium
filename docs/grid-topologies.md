...
*Status: **P0–P7 built.** `Aetherium.Server/Topology/` ships `IGridTopology`/`SquareTopology`/`HexTopology`/`TriangleTopology`/`H3Topology`/`GridTopologyRegistry`; every surveyed call site routes through the seam; the `world.topology` config field is threaded end-to-end (default `"square"`); `PerceptionDto` carries `Topology` + `SelfCellParity`; the validator enforces generator↔topology support; a hex-native `HexCavesGenerator` and the `hexhaven`/`trigrove` sample bundles ship under `Data/Games/`; the console renderer and Unity `GridMapView` place cells through the shared `Aetherium.Model.GridCellLayout`; and `H3Topology` (on the `pocketken.H3` NuGet package, registered `"h3"`) implements the spherical hexagon+pentagon tiling with hierarchy. The full suite (2230 tests) is green including golden-master square equivalence, hex/triangle/H3 end-to-end engine tests, the 8-invariant harness over every topology (incl. H3 pentagons), layout-vs-embedding equivalence, and the [pentagon-mock](h3-topology.md#the-pentagon-mock-already-in-ci) CI guard. The last mile for a *playable* H3 world (sphere-aware perception keys + an h3 generator) is documented in [h3-topology.md](h3-topology.md), gated on an actual planetary game concept. Grounded in the 2026-07-16 engine survey. The hexagon-specific deep dive lives in [hexagonal-tiles.md](hexagonal-tiles.md); this document supersedes its interface sketch with the generalized abstraction.*
...
**Built:** [docs/h3-topology.md](h3-topology.md) records the full stage-setting — the `WorldLocation` packing (H3's reserved top bit makes the 64-bit index split lossless), `cellToLocalIj` perception keys (preserving the opaque-string, no-absolute-coordinates contract on a sphere), hierarchy-via-portals posture, `Delta`-as-azimuthal-projection, worldgen by cell-center noise, and the `pocketken.H3` dependency assessment with a vendoring fallback. **Plus the code artifact**: [`PentagonishTopology`](../Aetherium.Test/Topology/PentagonishTopology.cs) — an intentionally irregular grid (y-even rows are 5-neighbor pentagons, y-odd rows 6-neighbor hexagons, cut symmetrically so neighbor symmetry holds) — runs through the [8-invariant harness](../Aetherium.Test/Topology/GridTopologyInvariants.cs) in CI as a cheap permanent guard that no uniform-direction (or global-plane) assumption regresses into the seam.

### P7 — H3 topology *(the drop-in)* — ✅ **built**

**Built:** [`H3Topology`](../Aetherium.Server/Topology/H3Topology.cs) implements `IGridTopology` + [`IHierarchicalGridTopology`](../Aetherium.Server/Topology/IHierarchicalGridTopology.cs) on the [`pocketken.H3`](https://github.com/pocketken/H3.net) NuGet package (pure C#, Apache-2.0, no native interop), registered as `"h3"`. Each cell packs its 64-bit H3 index into `GridCoord` X/Y (H3's reserved top bit keeps X non-negative); neighbors/distance/line/range delegate to H3's own spherical `gridDisk`/`gridDistance`/`gridPathCells`; `Delta` is the azimuthal projection of the great-circle displacement (scaled to cell-size units, so FOV/light math stays comparable), and H3 deliberately does **not** implement `IPlanarGridTopology`. Direction machinery is per-cell, so the 12 pentagons' 5-neighbor cells are handled by the same `AngularEdgeSelection` the triangle and pentagon-mock already proved. [`H3TopologyTests`](../Aetherium.Test/Topology/H3TopologyTests.cs) runs the full 8-invariant harness over a real H3 hex disc (via a new explicit-cell-set harness overload), adds pentagon-specific 5-neighbor coverage, checks the hierarchy nests (7 children per hex, parent round-trip), and drives `World.TryMoveSteps` end-to-end over an H3 world. **Remaining before a *playable* planetary world** (both gated on an actual game concept, neither touches the seam): the `cellToLocalIj` perception-key rerouting and an `h3` worldgen generator — see [h3-topology.md](h3-topology.md#what-remains-for-a-playable-h3-world).
...
No conversion of existing square content; no mixed topologies within one map; no live H3 *world* yet (the `H3Topology` tiling ships, but the sphere-aware perception keys and an h3 generator are gated on an actual planetary game — see P7); no pathfinding rework beyond neighbor substitution; no change to `WorldLocation`'s type, wire shape, or hashing; no FOV architecture change (per-cell raycast stays).

*(This non-goals list is as-of the original P0–P3 design round; P4–P7 have since built the hex generator, hex/triangle bundles, shared client layout math, and the `H3Topology` drop-in — see the per-phase sections above.)*
