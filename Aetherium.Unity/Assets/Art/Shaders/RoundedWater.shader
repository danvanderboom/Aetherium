Shader "Aetherium/RoundedWater"
{
    ...
    // UV1.x by WaterMeshBuilder: it carves a smooth curved edge (discard outside the
    // coast), fades shallows->deep, and draws an animated foam band hugging the shore.
    // _BandAlpha carries the band-stack depth falloff; _AmbientTint the frame's mood.
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Attributes
            {
                ...
                float2 uv0        : TEXCOORD0; // world XY (wave sampling)
            ...
            half4 frag (Varyings IN) : SV_Target
            {
                ...
                // Smooth curved coastline: fade to nothing just outside the coast.
                float edge = smoothstep(-_EdgeSoftness, _EdgeSoftness, d);
                if (edge <= 0.001)
                    discard;
