        ...
        public void DiscoverTypes(params Assembly[] assemblies)
        {
            ...
            foreach (var assembly in assemblies)
            {
                ...
                foreach (var type in generatorTypes)
                {
                    ...
                    {
                        // Prefer legacy Aetherium.MazeGenerator over WorldGen variant when both are present
                        if (string.Equals(name, "Maze", System.StringComparison.OrdinalIgnoreCase))
                        {
                            var existing = _generatorTypes[name];
                            var newIsLegacy = type.FullName != null && type.FullName.Equals("Aetherium.MazeGenerator");
